...
const NewsTabs: React.FC<NewsTabsProps> = ({ activeTab, onTabChange, extraTabs = [] }) => {
  // Merge default tabs with extraTabs (avoid duplicate values)
  const allTabs = [
    ...defaultTabs,
    ...extraTabs.filter(
      et => !defaultTabs.some(dt => dt.value === et.value)
    ),
  ];
  return (
