...
const ThreatMap: React.FC<ThreatMapProps> = ({ 
  ...
  const addIpMarker = useCallback(async (ip: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/ipgeo/${ip}`);
  ...
  const handleTraceroute = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/traceroute/${ipAddress}?t=${Date.now()}`);
    ...
    for (const line of hops.slice(0, 3)) {
      ...
      if (ipMatch) {
        ...
        try {
          ...
          const geoRes = await fetch(`http://localhost:5001/api/ipgeo/${ip}`);
